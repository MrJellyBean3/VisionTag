...
print(os.listdir("/dev/serial/by-id")[0])
arduino_id="/dev/serial/by-id/"+os.listdir("/dev/serial/by-id")[0]
print(os.path.exists(arduino_id))
working_dir=os.getcwd()
packages_path=working_dir+"/lib/python3.9/site-packages"
...
def capture_frames(camera_index, window_name,shared_frame):	#opencv image capture method
	...
	X_MIN2,X_MAX2,Y_MIN2,Y_MAX2=update_crop(camera_index,10,X_DIM,Y_DIM)
	t_l=time.time()
	while True:
		if (time.time()-t1s)>1:
			t1s=time.time()
...
if __name__=="__main__":
	...
	cam_setttings_file_name="camsettings.txt"
	cam_setttings_file=open(cam_setttings_file_name,"r")
	cam_settings=cam_setttings_file.readline()
	print("CAM SETTINGS: ",cam_settings)
	cam_setttings_file.close()
	cam2_on=True
	cam1_on=True
	using_double=False
	if cam_settings[0]=='f':
		...
		cam2_on=False
		pass
	elif cam_settings[0]=='c':
		pass
	elif cam_settings[0]=='b':
		pass
	...
	while (run_main_loop):
		...
		if health<=0:#check if dead
			...
			dead=False
		prev_switch_state=switch_on
		if (not (GPIO.input(38)))or(serial_arr[0]==1):	#switch on or off
			switch_on=True
		else:
			switch_on=False
		t2=time.time()
		...
		if shoot_or_no:	#send shot logic
			if cam_settings[0]=='f':
				frame=np.concatenate((shared_1["im"],shared_1["im2"]),axis=0)
			else:
