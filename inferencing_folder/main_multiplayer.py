            ...
            if (p1_shooting and not p1_dead)or(not p1_shooting and not p2_dead):
                try:
                    ...
                    if (len(masks_generated)>0):#if there are humans
                        ...
                        #convert mask to masked image
                        mask_cpu=mask_generated.cpu().numpy()
                        dims_t=mask_cpu.shape
                        im_data=cv2.imread(img)
                        im_data=cv2.cvtColor(im_data,cv2.COLOR_BGR2RGB)
                        dims_cat=(dims_t[0],dims_t[1])
                        out_data=cv2.resize(im_data,(dims_cat[1],dims_cat[0]))
                        mask_cpu=mask_cpu[:,:,np.newaxis]
                        mask_image=np.zeros((dims[0],dims[1],3))
                        #make masked image where the masked area is the regular image and the rest is black
                        mask_image[:,:,0]=mask_cpu[:,:,0]*out_data[:,:,0]
                        mask_image[:,:,1]=mask_cpu[:,:,0]*out_data[:,:,1]
                        mask_image[:,:,2]=mask_cpu[:,:,0]*out_data[:,:,2]
                        #turn into a tensor
                        mask_image=cv2.resize(mask_image,(640,448)) #width, height
                        cv2.imwrite("mask_test.jpg",mask_image)
                        mask_image_tensor=torch.tensor(mask_image).permute(2,0,1)
                        mask_image_tensor=mask_image_tensor.float()/255.0
                        mask_image_tensor=mask_image_tensor.unsqueeze(0)
                        mask_image_tensor=mask_image_tensor.to("cuda")
                        #categorize
                        with torch.no_grad():
                        ...
                        print("Predicted Class: ",predicted.item()," Confidence: ",confidence)
                        shared_data["category"]=predicted.item()

                        if (valid_hit>0.0) and (predicted.item()==0):#if there is a valid hit
